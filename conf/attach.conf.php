<?php

return array (
	'all'=> array('av','wmv','wav','wma','avi','rm','rmvb','mp4','webm','ogg','mp3','exe','bin','swf','fla','as','gif','jpg','jpeg','png','bmp','webp','doc','xls','ppt','docx','xlsx','pptx','pdf',
		'c','cpp','cc', 'txt','tar','zip','gz','rar','7z','bz','chm','bt','torrent','ttf','font','fon'
	),
	'video' => array('av','wmv','wav','wma','avi','rm','rmvb','mp4','webm','ogg'),
	'music' => array('mp3','mp4'),
	'exe' => array('exe','bin'),
	'flash' => array('swf','fla','as'),
	'image' => array('gif','jpg','jpeg','png','bmp','webp'),
	'office' => array('doc','xls','ppt','docx','xlsx','pptx'),
	'pdf' => array('pdf'),
	'text' => array('c','cpp','cc', 'txt'),
	'zip' => array('tar','zip','gz','rar','7z','bz'),
	'book' => array('chm'),
	'torrent' => array('bt','torrent'),
	'font' => array('ttf','font','fon'),
	'other' => array(),
);

?>