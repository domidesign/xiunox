...
本手册是**给人和 AI 一起用的**结构化多文件文档。本手册配套 [../SKILL.md](../SKILL.md) 作为简洁入口（含核心硬规则速查表 + 工作流 + 失败策略）。
...
| 维护官方插件市场仓库 | [13-official-plugins-repo.md](13-official-plugins-repo.md)（★插件市场清单与免费插件 zip 托管仓库的创建维护） |
...
| 给插件加"我的 xx"用户导航入口 | [19-user-nav.md](19-user-nav.md)（★用户导航注册：user_nav_register.php 一行接入首页右侧栏宫格 / 后台可排序启停） |
