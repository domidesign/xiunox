...
| 兼容层 | `xiuno-modern.js`（全局 `XN.*` API） | 提供 toast/ajax/escape/confirm |

## 禁止项（违反 = 扫描器拦截或运行异常）

- ❌ **禁止 jQuery**：新代码用 htmx 4 属性或原生 JS。旧代码里残留的 `$` 不推荐扩展。
...
- ❌ **禁止改核心 `model/*.func.php` 后不清 `tmp/model.min.php`**：OPCache 缓存旧编译，改完必须删除该文件（⚠️ 已违反 2 次）。
