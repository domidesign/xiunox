...
- 切换脚本在 [header.inc.htm:173-189](../../view/htm/header.inc.htm#L173-L189)，**内联同步执行**（禁止 `defer`，否则首屏 FOUC 闪烁）：
  ```php
...
- 后台管理：[admin/route/theme.php](../../admin/route/theme.php)，只支持 `light`/`dark` 两值，存 `conf/conf.php`。
...
- 生效靠 [view/css/theme.css:34-41](../../view/css/theme.css#L34-L41)，每个 brand 覆盖 Bootstrap 的 CSS 变量：
  ```css
  [data-theme="blue"] {
      --bs-primary: #2563eb;
  ...
  ```
- `<meta name="theme-color">` 由 [header.inc.htm:18-34](../../view/htm/header.inc.htm#L18-L34) 的 `$_theme_colors` 数组映射生成。
...
`overwrite/` 在编译期把核心模板**整个替换**成你的版本。机制见 [plugin.func.php:598-640](../../model/plugin.func.php#L598-L640) 的 `plugin_find_overwrite()`。
...
[plugin.func.php:615-619](../../model/plugin.func.php#L615-L619) 定义了白名单，以下路径 `overwrite` 会被静默拦截并记日志（`plugin_overwrite_error`）：
...
| 改主题切换脚本（[header.inc.htm:173-189](../../view/htm/header.inc.htm#L173-L189)）的 `defer` 行为 | FOUC 闪烁 |
...
通用检查表见 [SKILL.md 交付检查表](../xiunox-plugin-dev/SKILL.md)，以下为主题插件额外必查项：
...
| 首屏明暗闪烁（FOUC） | 主题切换脚本被加了 `defer`，必须内联同步执行（[header.inc.htm:171-172](../../view/htm/header.inc.htm#L171-L172)） |
...
> 通用开发流程、hook 目录、API 速查、安全规范见 [README.md](README.md) 与 [../xiunox-plugin-dev/SKILL.md](../xiunox-plugin-dev/SKILL.md)。
