...
| 分页 | 底部分页导航 |
| 左侧栏 | 版块导航（桌面端可见） |
| 右侧栏 | 用户信息卡片（登录用户）或站点信息卡片（游客）、快捷操作按钮等（桌面端可见） |
...
| 区域 | 说明 |
|------|------|
| 用户信息 | 头像、昵称、佩戴勋章、uid 与用户组徽章 |
| 积分徽章 | 显示当前积分（如「金币:120」），点击跳转我的积分页 |
| 用户导航宫格 | 两列快捷入口：内置的我的资料/我的积分/我的帖子/我的关注，及插件提供的入口（我的任务、我的答题、认证申请等） |
| 快捷按钮 | 「发新帖」及插件提供的签到等快捷操作（等宽并排） |

- 积分徽章、用户导航宫格均可点击跳转对应页面；宫格项悬停高亮、按压缩色
- 用户导航入口的显示项与顺序由站长在 后台 → 设置 → 导航 → 用户导航 统一配置
