...
**功能说明**：配置前台顶部导航栏、左侧导航栏、移动端底部导航、发现页应用宫格、用户导航宫格和页脚信息。页面包含六个子标签页。
...
**操作步骤**
1. 顶部开关控制移动端底部导航栏的整体启用/禁用
2. 在表格中编辑导航项的图标、选中图标、名称、样式类、URL
3. 勾选「需登录」的项仅对已登录用户显示（含游客项自动隐藏）
4. 点击「添加移动端导航项」新增导航，页面下方提供实时预览

**字段说明**
| 字段 | 类型 | 说明 |
|------|------|------|
| 图标 | text（图标选择器） | 默认状态图标，Tabler Icons 类名 |
| 选中图标 | text（图标选择器） | 选中状态图标 |
| 名称 | text | 导航项显示名称 |
| 样式类 | text | 附加 CSS 类 |
| URL | text | 导航项链接地址 |
| 需登录 | checkbox | 勾选后仅登录用户可见 |

#### 标签页 4：发现导航

**功能说明**：配置「发现」页面的宫格应用列表，每项对应一个应用入口。上半部分为可编辑的自定义项，下方为插件注册项（图标/名称/URL 只读展示，插件卸载后自动消失）。

**操作步骤**
1. 自定义项：编辑图标、名称、URL，点击「添加应用」新增入口，行末删除按钮移除
2. 插件注册项：默认启用，通过行首开关临时关闭（不删除，重新开启即恢复）
3. 拖拽行首图标调整顺序（自定义项与插件项可混排），点击「确认」保存

#### 标签页 5：用户导航

**功能说明**：配置首页右侧栏用户信息卡片下方的快捷入口（两列宫格，仅登录用户可见）。含系统内置入口（我的资料/我的积分/我的帖子/我的关注）与插件注册项（如我的任务、我的答题、认证申请等，由已启用插件自动提供）。

**操作步骤**
1. 通过行首开关启用/禁用任意入口（内置入口同样可关闭）
2. 拖拽行首图标调整显示顺序（内置入口与插件项可混排）
3. 点击「确认」保存，前台首页立即生效

**注意事项**
- 插件项的图标/名称/URL 为插件注册的只读值，不可编辑；URL 旁提供复制按钮
- 禁用插件后其入口自动从列表消失，重新启用后恢复原排序配置
- 内置入口行首为圆点图标，插件项行首为插头图标，便于区分
